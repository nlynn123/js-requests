...
const changeColorBack = (event) => {
    event.preventDefault
    document.getElementById("say-hello-button").style.backgroundColor = "white";
...
const repeatMyParam = () => {
    ...
    axios.get('http://localhost:3000/repeat/books')
    .then((res) => {
        console.log(res.data)
...
const query = (event) =>{
    event.preventDefault()
    axios.get('http://localhost:3000/query-test/?lions,tigers')
    .then((res) => {
        console.log(res.data)
    })
    .catch ((err) => {
...
form.addEventListener('click', createFood)